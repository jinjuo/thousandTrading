...
class strategy():
    ...
    def mail_watcher(self, msg):
        ...
        try:
            if time.time() - self.mail_time > 10 * 60:
    ...
    def GetTaLib(self):
        ...
        # boll
        self.upper, self.middle, self.lower = talib.BBANDS(self.close, timeperiod=30, nbdevup=2, nbdevdn=2, matype=talib.MA_Type.SMA)
    ...
    def HandleBar(self, mode):
        ...
        if 'macd' in mode:self.StrategyMacd()
    ...
    def Run(self):
        ...
        while True:
            try:
                ...
                self.mail_watcher(e)
                self.dingmessage('警告，出现错误', True)
                time.sleep(int(self.granularity)/10)
